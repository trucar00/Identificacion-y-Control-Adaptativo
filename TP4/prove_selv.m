...
% Inicializar RLS
P = 10000*eye(2);             
theta_hat = -0.3 * ones(2,1);
lambda = 0.99; % Factor de olvido
...
for i = 5:n
    ...
    theta_hat = theta_hat + K * (y(i) - y_hat);
    P = (P - K * x_k' * P) / lambda;
    ...
    u(i) = u(i-1)+t0*r(i)-s0*y(i)-s1*y(i-1); % Accion de control
...
figure;
plot(theta_hist(1,1:end), 'r', 'DisplayName', 'a_1', LineWidth=1.8);
hold on;
plot(theta_hist(2,1:end), 'b', 'DisplayName', 'b_1', LineWidth=1.8);
